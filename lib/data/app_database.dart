...
class AppDatabase extends _$AppDatabase {
  ...
  /// 이자 내역 id를 "자녀 + 주기 시작일"로 고정한다.
  /// 두 기기가 각자 자동 지급해도 동기화될 때 한 건으로 합쳐져 중복 지급을 막는다.
  ...
  Future<InterestBreakdown?> giveInterest(Child child, String editedBy,
    ...
    await _writeInterestTx(child, b, editedBy,
        _periodStart(child.interestPeriod, DateTime.now()), DateTime.now());
    return b;
  }

  /// 아무도 받지 않고 주기가 넘어갔을 때, 직전 주기 이자를 자동으로 지급한다.
  ...
  static const int tierSeedVersion = 4;
  ...
  static const List<(String, String, int, int, String, String, String?)> _defaultTiers = [
    ...
    // 횃불: 물과 석탄 사이에 새로 끼워넣은 등급(2026-08 서원 표 갱신). 기존 sav_07~27은
    // id/색상/아이콘 매핑을 그대로 두고 정렬순서(sortOrder)와 가격만 한 칸씩 밀었다.
    ('sav_28', 'savings', 7, 60000, '횃불', '🔥', 'Minecraft 30분 플레이권 1회(평일 가능)'),
    ('sav_07', 'savings', 8, 70000, '물', '💧', '아이스크림 & 음료'),
    ('sav_08', 'savings', 9, 80000, '안산암', '⬜', '원하는 책 1권'),
    ('sav_09', 'savings', 10, 90000, '돌', '🪨', '치킨 or 피자'),
    ('sav_10', 'savings', 11, 100000, '구리', '🟧', '치킨 or 피자 or 파스타 + 주간 저축률 10% 단위 open'),
    ('sav_11', 'savings', 12, 120000, '철', '⚙️', '마크 아이템/스킨'),
    ('sav_12', 'savings', 13, 140000, '금', '🟨', '키즈카페'),
    ('sav_13', 'savings', 14, 160000, '청금석', '🔵', '장난감(1.5만원 내)'),
    ('sav_14', 'savings', 15, 180000, '레드스톤', '🔴', '장난감(2만원 내)'),
    ('sav_15', 'savings', 16, 200000, '에메랄드', '💚', '원하는 식당 외식권'),
    ('sav_16', 'savings', 17, 250000, '다이아몬드', '💎', 'PPT or Excel 1시간'),
    ('sav_17', 'savings', 18, 300000, '네더라이트', '🖤', '레고 세트(4만원 내)'),
    ('sav_18', 'savings', 19, 350000, '흑요석', '🟪', '놀이공원 하루'),
    ('sav_19', 'savings', 20, 400000, '네더포탈', '🌀', 'Minecraft 1시간 플레이권 1회(평일 가능)'),
    ('sav_20', 'savings', 21, 500000, '신호기', '🔆', '원하는 학원 등록'),
    ('sav_21', 'savings', 22, 700000, '엔더 유적', '🏛️', '원하는 장난감(5만원 내)'),
    ('sav_22', 'savings', 23, 1000000, '엔더 포탈', '🌌', '서울 여행 원하는 곳'),
    ('sav_23', 'savings', 24, 1500000, '엔더 드래곤', '🐉', '원하는 장난감(10만원 내)'),
    ('sav_24', 'savings', 25, 2000000, '네더의 별', '🌟', '가족여행 선택권'),
    ('sav_25', 'savings', 26, 3000000, '엔드 크리스탈', '🔮', '원하는 장난감(20만원 내)'),
    ('sav_26', 'savings', 27, 5000000, '히로빈', '👤', '원하는 장난감(30만원 내)'),
    ('sav_27', 'savings', 28, 7000000, '모장', '🟥', '부모님과 협의'),
    // 모장 이후 신설 등급(2026-08). 마인크래프트 테마 밖이라 커스텀 이미지 없이 이모지만 쓴다.
    ('sav_29', 'savings', 29, 10000000, '스웨덴', '🇸🇪', '원하는 전자기기'),
    ('sav_30', 'savings', 30, 15000000, '대애애바아악부우우자아아', '💰', '원하는 장난감(40만원 내)'),
    ('sav_31', 'savings', 31, 20000000, '어떻게 여기까지 왔지?', '😱', '부모님과 자유 협의'),
