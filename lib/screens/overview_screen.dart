...
class OverviewScreen extends ConsumerWidget {
        ...
        children: [
                      ...
                      children: [
                        Expanded(
                          child: _BalanceMiniCard(
                            ...
                            balance: balance,
                            rate: rate,
                            interestAnnualPercent: _currentInterestAnnual(
                              ref,
                              balance,
                            ),
                          ),
                        ...
                        Expanded(
                          child: _WeeklyBudgetMiniCard(
                            ...
                            weeklyTiers:
                                ref.watch(weeklyTiersProvider).valueOrNull ??
                                const [],
                          ),
                        ),
                      ],
                ...
                children: [
                  for (final g in goals)
                    _GoalCard(
                      ...
                      onEdit: isChild
                          ? null
                          : () => _showGoalDialog(context, ref, existing: g),
                    ),
                ],
              );
            },
          ...
          if (showDetails) ...[
            ...
            expenseByCategoryAsync.when(
              ...
              data: (map) {
                if (map.isEmpty) {
                  return const _MutedCard(text: '아직 지출 내역이 없어요.');
                }
                final total = map.values.fold<int>(0, (a, b) => a + b);
                final entries = map.entries.toList()
                  ..sort((a, b) => b.value.compareTo(a.value));
                // 파이차트/범례에서 한 화면에 같이 보이는 카테고리끼리는 절대 색이
                // 겹치지 않도록 정렬 기반으로 배정한다("간식"/"기타" 색 겹침 문제).
            ...
            ref
                .watch(incomeByGiverProvider(child.id))
                .when(
            ...
            ref
                .watch(monthlyBreakdownProvider(child.id))
                .when(
                  loading: () => const _LoadingBox(height: 120),
                  error: (e, _) => Text('오류: $e'),
            ...
            ref
                .watch(yearlyStatsProvider(child.id))
                .when(
                  loading: () => const _LoadingBox(height: 120),
                  error: (e, _) => Text('오류: $e'),
          ...
          FilledButton(
            onPressed: () async {
              ...
              await ref
                  ...
                  .requestWishlist(child, title, amount, '아들');
              if (context.mounted) {
  ...
  Widget _buildRequestsSection(
    ...
    if (isChild) {
      final reqs =
          ref.watch(requestsProvider(child.id)).valueOrNull ?? const [];
    ...
    // 부모
    final pending =
        ref.watch(pendingRequestsProvider(child.id)).valueOrNull ?? const [];
  ...
  String _requestLabel(Request r) {
    ...
    if (r.type == 'stock') {
      return '주식: ${r.title ?? ''} ${formatWon(r.amount)}어치';
    }
  ...
  Widget _buildTierCard(WidgetRef ref) {
    final tierScore =
        ref.watch(summaryProvider(child.id)).valueOrNull?['tierScore'] ?? 0;
    final savingsTiers =
        ref.watch(savingsTiersProvider).valueOrNull ?? const [];
    ...
    final savePct = budget == 0
        ? 0
        : (((budget - spent) / budget) * 100).clamp(0, 100).round();
  ...
  void _showGoalDialog(BuildContext context, WidgetRef ref, {Goal? existing}) {
    ...
    final amountController = TextEditingController(
      text: existing != null ? '${existing.targetAmount}' : '',
    );
          ...
          if (isEdit)
            TextButton(
              onPressed: () async {
                ...
                await ref
                    .read(databaseProvider)
                    .softDeleteGoal(existing.id, owner);
...
class _WeeklyBudgetMiniCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final palette = appPalette(context);
    final scheme = Theme.of(context).colorScheme;
    final ratio = weeklyBudget == 0
        ? 0.0
        : (spent / weeklyBudget).clamp(0.0, 1.0);
    final over = remaining < 0;
    final barColor = over ? palette.expense.fg : palette.income.fg;
    final savePct = weeklyBudget == 0
        ? 0
        : ((remaining / weeklyBudget) * 100).clamp(0, 100).round();
            ...
            children: [
              ...
              ClipRRect(
                ...
                child: LinearProgressIndicator(
                  ...
                  backgroundColor: scheme.surfaceContainerHighest,
                  valueColor: AlwaysStoppedAnimation(
                    reached ? palette.special.fg : pair.fg,
                  ),
                ),
              ),
...
class _MonthlyBreakdownCardState extends State<_MonthlyBreakdownCard> {
  ...
  Widget build(BuildContext context) {
    ...
    var month = _month;
    if (month == null || !widget.byMonth.containsKey(month)) {
      month = months.last;
    }
