          ...
          if (child.interestEnabled) ...[
            ...
            Card(
              child: ListTile(
                ...
                subtitle: Text('주 3문제 · 해설 보고 다시 맞히면 '
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) {
          // 지금 설정으로 얼마가 나가는지 미리보기(약속 보너스는 뺀 기본 이자만).
          final preview = computeInterest(
            balance: balance,
            period: period,
            useBankRate: useBank,
                ...
                onPressed: () async {
                  ...
                  await ref.read(databaseProvider).updateChildPartial(
                        ...
                        ChildrenCompanion(
                          ...
                          interestUseBankRate: Value(useBank),
                          interestMultiplier: Value(mul),
                          updatedAt: Value(DateTime.now()),
                        ),
                      );
                  if (context.mounted) Navigator.pop(context);
        ...
        content: Column(
          ...
          children: [
            TextField(
              controller: controller,
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(labelText: '정답 1문제당(원)', hintText: '예: 10'),
            ),
            const SizedBox(height: 10),
            Text('주 3문제라 다 맞히면 이 금액의 3배를 받아요.\n'
