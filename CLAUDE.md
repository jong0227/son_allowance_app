...
- **v1.26.0** — 경제상식 주제 **67 → 101개** + **환율 계산기** 추가(경제왕 탭).
  - 빠져 있던 기본 개념을 채웠다: 희소성(기회비용의 전제인데 없었음), 경제 주체,
    ...
    포인트·배달비·휴대폰 요금, 리뷰/유명인 광고/무료 체험의 함정)도 보강.
  - 환율 계산기: 달러·엔·유로·위안 4종, 원↔외화 양방향 입력, 금액 버튼(1천~10만).
    엔화는 관행대로 100엔 기준 표시. 환율은 야후 `{통화}KRW=X`라 **API 키 불필요**
    (기존 `StockSearchService._fxToKrw`를 공개로 바꾸고 `fxRates()` 추가).
  - ⚠️ **실기기 확인 아직 못 함** — 아내 PC는 디버그 키라 릴리스 앱이 깔린 폰에
    설치가 막힌다(INSTALL_FAILED_UPDATE_INCOMPATIBLE). 노트북에서 릴리스 빌드로 확인할 것.
  - 💡 디버그/릴리스 공존 방법: `build.gradle.kts`의 `debug`에 `applicationIdSuffix = ".debug"`.
    단 Firebase 콘솔에 `com.moneycraft.app.debug` 앱을 등록하고 google-services.json을
    갱신해야 빌드가 통과한다(google-services 플러그인이 패키지 일치를 요구). 아직 미적용.
...
- **경제상식**: **101주제**(`lib/data/economy_topics.dart`) — 데이터만 추가하면 화면은 공용 렌더러 재사용.
...
- **환율 계산기**(`exchange_calculator_screen.dart`): 달러·엔·유로·위안. 원↔외화 양방향.
  통화 목록은 `market_provider.dart`의 `kFxCurrencies`(야후에 시세가 있는 것만 — 베트남 동은 없음).
